...
This document describes the GNOME platform application. Run its commands from
the repository root. Agents Tray Limits is a GNOME Shell extension that shows
Codex and Claude Code subscription rate limits in the top panel. One indicator
follows the selected profile, while its menu summarizes every configured
profile and switches between them instantly.
